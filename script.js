...
const COMMITS_FOLDER_PATH = process.env.COMMITS_FOLDER_PATH ?? "./history-tracker";
...
async function generateGitCommits(commits) {
  ...
  // Ensure the history subfolder directory exists before writing to it
  if (!fs.existsSync(COMMITS_FOLDER_PATH)){
      fs.mkdirSync(COMMITS_FOLDER_PATH, { recursive: true });
  }

  let i = 1;
  console.log("Starting code history commitment logs...");
  ...
  for (const commit of commits) {
    ...
    await execAsync(`git add ${COMMITS_FOLDER_PATH}/README.md`);
    
    const cleanComment = commit.comment.replace(/"/g, '\\"');

    //Multiplatform inline-injection mapping for Git timeline overrides
    if (process.platform === "win32") {
        await execAsync(`set GIT_AUTHOR_DATE="${formattedDate}" && set GIT_COMMITTER_DATE="${formattedDate}" && git commit -m "${cleanComment}" --date "${formattedDate}"`);
    } else {
        await execAsync(`GIT_AUTHOR_DATE="${formattedDate}" GIT_COMMITTER_DATE="${formattedDate}" git commit -m "${cleanComment}" --date "${formattedDate}"`);
    }
  ...
  console.log(`Successfully locally registered all ${commits.length} commits.`);
  
  // Comment this out if you are running from LOCAL env
...
async function main() {
  ...
  await generateGitCommits(COMMITS)
}
